...
> 注意，这是有关于 `min()` 、`max()` 和 `clamp()` 函数最基本的使用，但这里不深入介绍，更详细的内容将会放在后面的“[[构建响应式 UI|响应式 UI]]”。
...
你可能现在不知道 CSS 容器查询 `@container` 特性是怎么一回事，那也不用担心，因为我们在介绍下一代响应式 Web 设计的时候有专门介绍该特性。如果你实在等不及，可以先行了解 CSS [[容器查询]]特性是什么东东！
