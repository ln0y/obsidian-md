...
![](_attachment/img/795f67fff84f43d881b8412dfa0c7311_MD5.gif)
...
![](_attachment/img/bee89fcb44225d2f7741c40d1d5a0846_MD5.png)
...
![](_attachment/img/b56212d610f9b69437d792dda8a932f6_MD5.png)
...
![](_attachment/img/f8126b980e83eb604598175757a124f8_MD5.png)

依此类推，最终你可能创建的是一个 ` 5 x N` 的网格（`N` 是行网格轨道数量）:

![](_attachment/img/22832d40965e093208611bc304883ddb_MD5.gif)
...
![](_attachment/img/f3e8fb4c644cc15f7bb0e4091f03315d_MD5.png)
...
![](_attachment/img/3381d9a3728056a77550662fb7122744_MD5.png)
...
![](_attachment/img/6de43af19ba6846e76549a49b9faeffa_MD5.png)
...
![](_attachment/img/d47afbec2ccac5521d0eeeea8d1957c7_MD5.png)

但是我们可以换一种思路来理解，它可以是两条网格线，只不过前者是以 `-end` 命名的网格线，后者是 `-start` 命名的网格线：

![](_attachment/img/837a4bbea08fcb3f437af84670e353cf_MD5.png)
...
![](_attachment/img/ccdaf215bee558f61b7ebc32a0f3bdf6_MD5.png)
...
![](_attachment/img/b5243fea88cd08027353c260901c4f29_MD5.png)
...
![](_attachment/img/b53b0cc7fe360556c55b0586aca70aea_MD5.png)
...
![](_attachment/img/ec20d82da089aa8daf76c55c6cefc715_MD5.png)
...
![](_attachment/img/88cc55d7c0103542e3002514bae2057a_MD5.png)

如此一来，你可以使用 `grid-template-areas` 结构化（可视化）来构建 Web 布局，比如我们常见的一个 Web 布局：

![](_attachment/img/1cc3b5487e6694c1af34eff0c31f11f2_MD5.png)
...
![](_attachment/img/8fe1b146b56447d932bb88b7ca1b5f7d_MD5.png)
...
![](_attachment/img/e944af43f963add604234b96309722ad_MD5.png)
...
![](_attachment/img/7327c8784ff3623cd5bdb3dbb2ca2d9f_MD5.png)

使用 **规则 ①** 可以实现区域填充网格且不留空余空间，但不能使用 **规则 ②** 来给网格留出一个空的或几个空的单元格。可实际生产中，Web 布局有时的确需要有空的单元格存在。比如下图这样的 Web 布局：

![](_attachment/img/c5dc3229d6b502bd55955dce0e3c8ad2_MD5.png)
...
![](_attachment/img/b8c40cf6d1d46a80d826e3b49acd18a3_MD5.png)
...
![](_attachment/img/e1c3aed06c193823a20f35e6d0aff1c2_MD5.png)
...
![](_attachment/img/c45b05bd209fbf0003b31523953e609f_MD5.png)

> Demo 地址： <https://codepen.io/airen/full/vYjQvNo>

第四条规则：

> **规则 ④ ： 多个相同单元格命名（令牌）创建一个具有相同名称的命名网格区域。简单地说，跨行或列 命名相同的网格区域名称，可以达到合并单元格的作用**。

其实，前面的示例中已经有 **规则 ④** 存在了。这里还是单独拿一个示例来介绍，大家更易于理解。例如下图这个示例，需要将侧边栏（`sidebar`）和页脚（`footer`）合并起来（`main` 和 `footer` 区域具有相同的宽度）：

![](_attachment/img/90e09421c38380c98faf54463216105f_MD5.png)
...
![](_attachment/img/974fb8649b7ca647b3b5fb1fc8fa3b3d_MD5.gif)

> Demo 地址：<https://codepen.io/airen/full/BaxGMmx>

两者差异如下图所示：

![](_attachment/img/b17f13438678855883f4798b039d99c5_MD5.png)
...
![](_attachment/img/6f609f8e1b5429d7e7bd0a06b2bd28fe_MD5.png)

**规则 ⑤** 提到了，给网格区域命名并不是任何名称都是有效的：

> **规则 ⑤ ：任何其他字符的序列，会代表一个垃圾标记（Trash Token），会使声明无效。**

在显式给网格区域命名时，尽可能使用有语义的名称，比如前面示例中所示的 `header` 、`nav` 等等。切勿使用一些数字（开头）、标点符号以及它们的组合等，比如 `1` 、`#` 、`1st` 等，将会被视为无效的声明。如果命名的名称是无效的，那么 `grid-template-areas` 属性也会被视为无效：
...
![](_attachment/img/b564dae8cddb4f2fddfe0c45aadbf807_MD5.png)
...
![](_attachment/img/63a2caf20db345a3e34c748695ecfe31_MD5.png)
...
![](_attachment/img/20293f9355aff6873191ccbe15554af6_MD5.gif)
...
通过前面的学习，我们知道了什么是显式网格，以及如何定义一个显式网格。在 CSS 网格布局中，除了使用 `grid-template-*` （`grid-template-columns` 、`grid-template-rows` 和 `grid-template-areas`）之外，还可以使用 `grid-auto-*` （即 `grid-auto-columns` 、`grid-auto-rows` 和 `grid-auto-flow`）来定义一个网格。
...
![](_attachment/img/0e8f645175aa69b2918c9a929aa686d1_MD5.gif)
...
![](_attachment/img/d08adac28acd298ec0c2be2bd53d8ef0_MD5.png)
...
![](_attachment/img/fbb5abef39f9a7a107118b23e0116b02_MD5.png)
...
![](_attachment/img/b9b069b3f7d500ea9de08f806827d51e_MD5.png)
...
![](_attachment/img/e847a6389014ab38fbf65635c379f69b_MD5.png)
...
![](_attachment/img/93188e91bf97bdf9d86e4cefe22e7509_MD5.gif)

> Demo 地址： <https://codepen.io/airen/full/gOzZGvX>

即网格流受书写模式影响：

![](_attachment/img/b09cb3eb24c9525563a40bf00e9d9f35_MD5.png)
...
![](_attachment/img/7569d55fcc45608aa48631bdb0c2f7e5_MD5.gif)

当 `grid-auto-flow` 取值为 `column` 时，网格项目自动放置会从第一列第一个网格单元格开始，从上往下排列，当网格行轨道数量不够时，会新创建一列网格轨道：

![](_attachment/img/314c80a583176ac9a95dbad665ccd709_MD5.gif)

`grid-auto-flow` 取值为 `row` 和 `column` 时网格项目自动放置流方向对比如下：

![](_attachment/img/b1af2c376f7faa2a55aeea34acefc2f9_MD5.png)
...
![](_attachment/img/23595350e3a809746bb4290c3efccb30_MD5.png)
...
![](_attachment/img/4e840fc6e10abd7fba4c46da1e703705_MD5.gif)

`row` 、`column` 、`dense` 、`row dense` 和 `column dense` 效果对比如下：

![](_attachment/img/90142cd511e12d0ba65f2d233e0cec02_MD5.png)
...
![](_attachment/img/eb7e6900b6b1701111cbe1c698b7a666_MD5.png)
