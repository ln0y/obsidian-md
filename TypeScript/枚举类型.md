---
aliases: ['enums']
tags: ['ts/enums','TypeScript','date/2022-02','year/2022','month/02']
date: 2022-02-18-Friday 14:07:55
update: 2022-02-18-Friday 14:29:12
---
...
- [[#数字枚举|数字类型]]
- [[#字符串枚举|字符串类型]]
- [[#异构枚举（Heterogeneous enums）|异构类型]]
- [[#常量成员和计算（值）成员]]
- [[#枚举成员类型和联合枚举]]
- [[#常量枚举（const enums）|常量枚举]]
- [[#外部枚举（Ambient enums）|外部枚举]]
...
当然，异构枚举也被认为是很“鸡肋”的类型。比如如下示例中，我们定义了成员 SUNDAY 是 'SUNDAY'、MONDAY 是 2。
...
此外，回想[[字面量类型]]特性，不同成员的类型就是不同的字面量类型。纯字面量成员枚举类型也具有字面量类型的特性，也就等价于枚举的类型将变成各个成员类型组成的联合（枚举）类型。

[[联合类型]]使得 TypeScript 可以更清楚地枚举集合里的确切值，从而检测出一些永远不会成立的条件判断（俗称 Dead Code），如下所示示例（援引自官方恒为真的示例）：
...
此外，[[字面量类型]]所具有的[[类型推断]]、[[类型收窄|类型缩小]]的特性，也同样适用于字面量枚举类型，如下代码所示：
