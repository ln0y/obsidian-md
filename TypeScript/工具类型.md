...
date: 2022-02-20-Sunday 18:29:23
update: 2022-02-20-Sunday 22:13:42
---
...
在上述示例中，Pick工具类型接收了两个[[泛型]]参数，第一个 T 为给定的参数类型，而第二个参数为需要提取的键值 key。有了参数类型和需要提取的键值 key，我们就可以通过映射类型很容易地实现 Pick 工具类型的功能。
...
TypeScript 自 4.1版本起开始支持模板字符串[[字面量类型]]。为此，TypeScript 也提供了 Uppercase、Lowercase、Capitalize、Uncapitalize这 4 种内置的操作字符串的类型，如下示例：
