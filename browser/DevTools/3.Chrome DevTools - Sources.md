...
目前越来越多网页的代码都会经过处理，实际使用时和开发时看到的并不同，而处理过的代码通常会被压缩，几乎不能用来 Debug，因此使用 Sources 面板进行 Debug 时常常会需要 Source map 来映射目前执行的代码到源代码上，关于 Source map 更多的介绍可以参考 [Sources - Files](https://ithelp.ithome.com.tw/articles/10242922)。
