...
以下图为例，可以看到文件原始大小为 1.2 MB，但网路实际传输量只有 377 kB，一般网页的资源都在传输前进行压缩，再传送到前端，可以大幅缩小实际传输的流量，常见的压缩格式有 `gzip`、`deflate`、`Brotli` 等等，在请求时可以用 `accept-encoding`Header 列出浏览器支援的压缩格式，回应则会以 `content-encoding`Header 表示该资源使用的压缩格式。
