...
因为是我们只是简单的示范，所以这里会一直无限循环，实际上通常会用在断线重连的情境。
...
从上图可以看到后续重新订阅的行为就被延后了，但实际上我们不太会用 retryWhen 来做重新订阅的延迟，通常是直接用 catch 做到这件事。这里只是为了示范 retryWhen 的行为，实际上我们通常会把 retryWhen 拿来做错误通知或是例外收集，如下
