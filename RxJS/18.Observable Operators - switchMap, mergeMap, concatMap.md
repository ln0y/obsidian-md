...
这个示例的外部 observable 送出的元素就是 click event 对象，内部 observable 送出的元素就是 response 对象，这里我们回传 response 对象的 title 属性，这样一来我们就可以直接收到 title，这个方法很适合用在 response 要选取的值跟前一个事件或顺位 (index) 相关时。
...
如果我们快速的连续点击五下，大家可以在开发者工具的 network 看到每个 request 会在点击时发送并且会 log 出五个对象，如下图
...
这三个 operators 还有一个共同的特性，那就是这三个 operators 可以把第一个参数所回传的 promise 对象直接转成 observable，这样我们就不用再用 `Rx.Observable.from` 转一次，如下
