---
aliases: []
tags: ['DNS','Network','date/2022-04','year/2022','month/04']
date: 2022-04-16-Saturday 17:33:30
update: 2022-04-16-Saturday 17:50:37
---
...
![[Cgp9HWCk6Z-ASLtEAAH_Ssm7xjk737.png|800]]
...
![[CioPOWCk6auABUkFAAKhjoCbm9k527.png|800]]
...
![[Cgp9HWCk6bGABepRAAHGRO0l88o350.png|800]]
...
以lagou为例，如果在某个时刻同一个区域内有一个用户触发过上述 1~8 的过程，另一个同区域的用户就可以在本地 DNS 服务器中获得 DNS 记录，而不需要再走到根 DNS 服务器。这种设计，我们称作**分级缓存策略**。
...
www.example.com 是要解析的域名。A 是记录的类型，A 记录代表着这是一条用于解析 IPv4 地址的记录。从这条记录可知，www.example.com 的 IP 地址是 139.18.28.5。`;`是语句块的结尾，也是注释。
...
在工作中，我们经常会发邮件到某个同事的邮箱。比如说，发送一封邮件到 xiaoming@gmail.com，那么谷歌网如何知道哪个 IP 地址是邮件服务器呢？
...
这样凡是 @gmail 的邮件都会发送到 mail.gmail.com 中，而 mail.gmail.com 的 IP 地址可以通过查询 mail.gmail.com 的 A 记录和 AAAA 记录获得。
