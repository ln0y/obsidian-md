...
update: 2023-01-03-星期二 18:34:29
---
...
![[_attachment/img/169e1b7893b5b5c7_tplv-t2oaga2asx-zoom-1.png]]

这是为啥呢？这就要从网络请求的整个流程说起了。一般情况下，当我们访问一个 url 的时候，会发生下图的过程：

![[_attachment/img/169ecd30b1247ec6_tplv-t2oaga2asx-zoom-1.png]]
...
![[_attachment/img/169ecdacec73af47_tplv-t2oaga2asx-zoom-1.png]]
...
我们选取业务开发中最常见的四种代理类型：事件代理、虚拟代理、缓存代理和保护代理来进行讲解。

在实际开发中，代理模式和观察者模式一样，可以玩出花来。但设计模式这玩意儿就是这样，变体再多、玩得再花，它的核心操作都是死的，套路也是死的——正是这种极强的规律性带来了极高的性价比。

### 事件代理

事件代理也可以叫 [[事件委托]]，可能是代理模式最常见的一种应用方式，也是一道实打实的高频面试题。它的场景是一个父元素下有多个子元素，像这样：
...
![[_attachment/img/169e33b9b6895e00_tplv-t2oaga2asx-zoom-1.png]]

我们发现 proxyAddAll 针对重复的入参只会计算一次，这将大大节省计算过程中的时间开销。现在我们有 6 个入参，可能还看不出来，当我们针对大量入参、做反复计算时，缓存代理的优势将得到更充分的凸显。

### 保护代理

开婚介所的时候，为了保护用户的私人信息，我们会在李雷访问小美的年龄的时候，去校验李雷是否已经通过了我们的实名认证；为了确保~婚介所的利益~李雷确实是一位有诚意的男士，当他想获取小美的联系方式时，我们会校验他是否具有 VIP 资格。所谓“保护代理”，就是在访问层面做文章，在 getter 和 setter 函数里去进行校验和拦截，确保一部分变量是安全的。
...
第一个参数是我们的目标对象，也就是上文中的“小美”。handler 也是一个对象，用来定义 **代理的行为**，相当于上文中的“婚介所”。当我们通过 proxy 去访问目标对象的时候，handler 会对我们的行为作一层拦截，我们的每次访问都需要经过 handler 这个第三方。

#### “婚介所”的实现

小美的个人信息，刚问了下我们已经注册了 VIP 的李雷，大致如下：
...
值得一提的是，Proxy 它本身就是为拦截而生的，所以我们目前实现保护代理时，考虑的首要方案就是 ES6 中的 Proxy。

## 小结

代理模式行文至此，相信大家都已经做到了心中有数。我们看到代理模式的目的是十分多样化的，既可以是为了加强控制、拓展功能、提高性能，也可以仅仅是为了优化我们的代码结构、实现功能的解耦。无论是出于什么目的，这种模式的套路就只有一个—— A 不能直接访问 B，A 需要借助一个帮手来访问 B，这个帮手就是代理器。需要代理器出面解决的问题，就是代理模式发光发热的应用场景。
