...
date: 2023-07-10-星期一 17:08:22
update: 2023-07-10-星期一 17:46:07
---

函数式编程是一门有着复杂数学背景的学问，它背靠的数学理论非常多，但最关键的理论无外乎以下三个：
...
2. **函数组合**：在 Lambda 演算中，我们可以通过函数组合来构建更复杂的计算。例如，我们可以将两个函数 f 和 g 组合成一个新函数 `λx.(f (g x))`：给定函数 `f = λx.(x + 1) 和 g = λx.(x _ 2)` _，_ 我们可以创建一个新函数 `λx.((x _ 2) + 1)`，它会先将输入值乘以 2，然后加 1。
...
顾名思义，“匿名函数”当然就是没有名字的函数。在 Lambda 演算中，**匿名函数指的其实就是 Lambda 表达式**。在我们楼上的示例中，函数组合中的 `f = λx.(x + 1)` 和 `g = λx.(x _ 2)` 都是具名函数。但如果我们把 f、g 这两个名字拿掉，那么 `λx.(x + 1)` 和 `λx.(x _ 2)` 这两个函数就都是匿名函数了。
...
在这个例子中，我们定义了组合子函数 S、B 和 C，它们分别对应于 Lambda 演算中的 S、B 和 C 组合子。这些组合子能够实现几种不同方式的函数复合。其中 B 组合子最为经典，有心的同学可以看出来，它其实就是我们曾经实现过的“compose 的最小单元函数”。
...
> 注：pipe 和 compose 的推导过程，在 [深入函数组合(Composition)思想](JavaScript/functional%20programming/深入函数组合(Composition)思想.md) 有详尽的分析，此处仅作复习，不多赘述。
...
> 注：curry 的推导过程，在 [偏函数 & 柯里化](JavaScript/functional%20programming/偏函数%20&%20柯里化.md) 有详尽的分析，此处仅作复习，不多赘述。
...
在之前我们系统地学习了范畴论中的核心实践方法。作为函数式编程中的关键数学依据，我们在本节仍然有必要对范畴论作整体的回顾和总结。

### 当我们谈论范畴论时，我们在谈论什么

对于复杂的数学理论，我们不必执着于理论本身，而是应该**从代码去推导数学**。
...
当我们在函数式编程的语境中谈论范畴论时，数学理论本身是最不重要的一个部分。

### 从范畴论到 JS 函数式编程实践

在 JavaScript 函数式编程中，范畴论的实践主要体现在对 [Functor](JavaScript/functional%20programming/Functor.md)、[Monad](JavaScript/functional%20programming/Monad.md)、[Semigroup](JavaScript/functional%20programming/Semigroup.md) 和 [Monoid](JavaScript/functional%20programming/Monoid.md) 这四种结构的应用。下面我将结合具体的代码，和大家一起对这四种结构的特征进行回顾。
...
const Task = run => ({
  ...
  flatMap: f =>
    Task((resolve, reject) => run(value => f(value).run(resolve, reject), reject)),
...
// foldMap 实现
const foldMap = (f, list) => list.map(f).reduce((acc, x) => acc.concat(x), f.empty())
