...
date: 2023-07-11-星期二 16:49:08
update: 2023-07-11-星期二 16:51:34
---

当下，[盒子模式](JavaScript/functional%20programming/范畴论.md) 在前端函数式编程的实践主要有两个方向：一是**响应式编程库**，如 RxJS、Bacon.js 和 Most.js 等库，它们将异步数据流视为盒子；另一类则是**函数式编程工具库**，如 fp-ts、Folktale 和 Sanctuary 等库，它们提供了对 Functor、Monad 等抽象的数据结构和相关操作的实现。

在 [函数式编程沉思录（下）：从数学理论到编码实践](JavaScript/functional%20programming/函数式编程沉思录（下）：从数学理论到编码实践.md)，我们已经对盒子模式在 RxJS 中的应用有了一定的探讨和分析。实际上，**像 RxJS 这样的响应式编程库，一般都会实践盒子模式，因为它们需要借助盒子模式实现流（Stream）的抽象**——流（Stream）通常会被封装为一个盒子，如 RxJS 中的 `Observable`。
...
通过 [函数式思想在前端状态管理中的实践：盒子模式 & RxJS 篇](JavaScript/functional%20programming/函数式思想在前端状态管理中的实践：盒子模式%20&%20RxJS%20篇.md) 的学习， 我们已经知道，**RxJS 中的** `Observable` **既是一个 Functor，也是一个 Monad**。
...
在我提到的这两个最明显的共性中， **“都提供了 Maybe/Option 和 Either 这两类盒子**”是一个很有趣的点。Maybe 盒子我们在 [Functor](JavaScript/functional%20programming/Functor.md) 已经介绍过，接下来我们通过一个例子来认识一下 Either 盒子:
